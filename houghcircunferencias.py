...
# Se aplica  la transformada de Hough para detectar circunferencias
...
cv2.imshow('Imagen que incluye la circunferencia principal', imagen_escala_grises)
cv2.waitKey(0)
cv2.imshow('Imagen que incluye la circunferencia principal', imagen)
cv2.waitKey(0)
...
plt.title('Imagen que incluye la circunferencia principal')

plt.subplot(1, 2, 2)
plt.imshow(cv2.cvtColor(bordes, cv2.COLOR_BGR2RGB))
plt.title('Imagen procesada a la cual se le detecta la circunferencia principal')
